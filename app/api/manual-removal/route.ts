// /app/api/manual-removal/route.ts
...
export async function POST(req: NextRequest) {
  try {
    ...
    await createStockMovement({ productId, type: "MANUAL_REMOVAL", quantity, storeId });

    return NextResponse.json(
      { message: "Stock removed successfully", product: updatedProduct },
      { status: 200 }
    );
  } catch (error) {
