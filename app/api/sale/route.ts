  ...
  try {
    ...
    try {
      ...
      if (error instanceof Error && error.message === "Insufficient stock") {
        return NextResponse.json({ message: "Insufficient stock for sale" }, { status: 400 });
      }
      throw error;
    }

    // Record the stock movement
    await createStockMovement({ productId, type: "SALE", quantity });

    return NextResponse.json({ message: "Sale recorded successfully", product: updatedProduct }, { status: 200 });
  } catch (error: unknown) {
